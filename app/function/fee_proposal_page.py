...
class FeeProposalPage(tk.Frame):
    ...
    def reference_part(self):
        ...
        tk.Entry(reference_frame, width=70, textvariable=reference["Date"], font=self.app.font).grid(row=0, column=1, padx=(0, 10))
        ...
        tk.Entry(reference_frame, width=70, textvariable=reference["Revision"], font=self.app.font).grid(row=1, column=1, padx=(0, 10))
    def time_part(self):
        time = dict()
        ...
        for i, stage in enumerate(stages):
            ...
            tk.Entry(time_frame, width=20, font=self.app.font, fg="blue", textvariable=time[stage]["Start"]).grid(row=i, column=1)
            tk.Label(time_frame, text="-").grid(row=i, column=2)
            tk.Entry(time_frame, width=20, font=self.app.font, fg="blue", textvariable=time[stage]["End"]).grid(row=i, column=3)
            tk.Label(time_frame, text=" business day", font=self.app.font).grid(row=i, column=4)
    def scope_part(self):
        scope = dict()
        self.data["Fee Proposal"]["Scope of Work"] = scope
    ...
    def fee_part(self):
        ...
        self.data["Fee Proposal"]["Financial"] = fee
        ...
        top_frame.pack()
        tk.Label(top_frame, text="", width=6, font=self.app.font).grid(row=0, column=0)
        tk.Label(top_frame, width=50,  text="Services", font=self.app.font).grid(row=0, column=1)
        tk.Label(top_frame, width=20,  text="ex.GST", font=self.app.font).grid(row=0, column=2)
        tk.Label(top_frame, width=20,  text="in.GST", font=self.app.font).grid(row=0, column=3)

        bottom_frame = tk.LabelFrame(self.fee_frame)
        bottom_frame.pack(side=tk.BOTTOM)
        tk.Label(bottom_frame, text="", width=6, font=self.app.font).grid(row=0, column=0)
        tk.Label(bottom_frame, width=50,  text="Total", font=self.app.font).grid(row=0, column=1)
        tk.Label(bottom_frame, width=20,  textvariable=fee["Fee"], font=self.app.font).grid(row=0, column=2)
        tk.Label(bottom_frame, width=20,  textvariable=fee["in.GST"], font=self.app.font).grid(row=0, column=3)
    def update_scope(self, var):
        scope = self.data["Fee Proposal"]["Scope of Work"]
        service = var["Service"].get()
        include = var["Include"].get()
        if include:
            self.scope_frames[service] = tk.LabelFrame(self.scope_frame, text=service, font=self.app.font)
            ...
            for i, extra in enumerate(extra_list):
                extra_frame = tk.LabelFrame(self.scope_frames[service], text=extra, font=self.app.font)
                extra_frame.pack()
                ...
                for j, context in enumerate(items):
                    ...
                    tk.Entry(extra_frame, width=100, textvariable=scope[service][extra][j]["Item"], font=self.app.font, bg=color_list[j%2]).grid(row=j, column=1)
                # garbage collection
    ...
    def update_fee(self, var):
        details = self.data["Fee Proposal"]["Financial"]["Details"]
        service = var["Service"].get()
        include = var["Include"].get()
        if include:
            details[service] = {
            ...
            self.fee_dic[service] ={
                ...
                "Expand": tk.Checkbutton(self.fee_frames[service],
                                         text="Expand",
                                         variable=details[service]["Expand"],
                                         font=self.app.font),
                "Content":{
                    "Details":[
                        {
                    ...
                    "Service":tk.Label(self.fee_frames[service], width=36, text=service + " Total", font=self.app.font),
            ...
            details[service]["Expand"].trace("w", lambda a, b, c: self._expand(service))

            details[service]["Fee"].trace("w", lambda a, b, c:self.app._ist_update(details[service]["Fee"], details[service]["in.GST"]))
            details[service]["Fee"].trace("w", lambda a, b, c: self.app._sum_update(
                [value["Fee"] for value in details.values()], self.data["Fee Proposal"]["Financial"]["Fee"]))
            details[service]["in.GST"].trace("w", lambda a, b, c: self.app._sum_update(
                [value["in.GST"] for value in details.values()], self.data["Fee Proposal"]["Financial"]["in.GST"]))
            ...
            for i in range(self.app.configuration["n_items"]):
                ...
                sum_fun = lambda a, b, c: self.app._sum_update([item["Fee"] for item in details[service]["Content"]],
                                                               details[service]["Fee"])
                details[service]["Content"][i]["Fee"].trace("w", sum_fun)
        else:
    ...
    def _append_value(self, service, extra):
        scope = self.app.data["Fee Proposal"]["Scope of Work"]
        ...
        tk.Entry(self.scope_frames[service].winfo_children()[index], width=100,
                 ...
                 font=self.app.font).grid(row=len(scope[service][extra]), column=1)
