...
all_new_sub_task = clean_response(task_api_instance.get_subtasks_for_task(new_project_gid))


for i, e in enumerate(all_new_sub_task):
    new_task_gid = e["gid"]
    new_task = clean_response(task_api_instance.get_task(new_task_gid))
...
all_old_sub_task = clean_response(task_api_instance.get_subtasks_for_task(old_project_gid))
for i, e in enumerate(all_old_sub_task):
    old_task_gid = e["gid"]
    old_task = clean_response(task_api_instance.get_task(old_task_gid))
