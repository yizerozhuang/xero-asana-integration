...
class FinancialPanelPage(tk.Frame):
    ...
    def invoice_part(self):
        ...
        tk.Label(total_frame, width=10, textvariable=self.data["Invoices"]["Fee"], font=self.conf["font"]).grid(row=0, column=1)
        tk.Label(total_frame, width=10, textvariable=self.data["Invoices"]["in.GST"], font=self.conf["font"]).grid(row=1, column=1)
        ...
        if include:
            ...
            self.invoice_dic[service] = {
                ...
                "Fee": tk.Label(self.invoice_frames[service],
                                textvariable=self.data["Invoices"]["Details"][service]["Fee"],
                                width=10,
                                font=self.conf["font"]),
                "in.GST": tk.Label(self.invoice_frames[service],
                                   textvariable=self.data["Invoices"]["Details"][service]["in.GST"],
                                   width=10,
                                   font=self.conf["font"]),
    ...
    def generate_invoice_number(self):
        invoice_number_file = os.path.join(self.conf["resource_dir"], "xlsx", "Invoice Number.txt")
        cur_invoice_number_list = [int(invoice.strip("\n")) for invoice in open(invoice_number_file).readlines()]
        new_invoice_number = max(cur_invoice_number_list) + 1
        self.data["Financial Panel"]["Invoice Details"]["INV1"]["Number"].set(str(new_invoice_number))
        self.invoice_dic["Number"]["INV1"].config(state=tk.DISABLED)
